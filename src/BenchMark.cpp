...
void BenchmarkMR_malloc(size_t ntimes, size_t nworks, size_t rounds, vector<int>& test)
{
	...
	printf("采用MR_malloc策略：%zu个线程并发执行%zu轮，每轮申请空间 %zu次: 花费：%zu ms\n",
		nworks, rounds, ntimes, malloc_costtime.load());

	printf("采用MR_malloc策略：%zu个线程并发执行%zu轮，每轮释放空间 %zu次: 花费：%zu ms\n",
		nworks, rounds, ntimes, free_costtime.load());

	printf("采用MR_malloc策略：%zu个线程并发%zu次，总计花费：%zu ms\n",
		nworks, nworks * rounds * ntimes, malloc_costtime.load() + free_costtime.load());
}
